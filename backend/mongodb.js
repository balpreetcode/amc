...
async function validateSessionToken(token) {
    ...
    try {
        ...
        // Check if session is active - if no status fields exist, treat as active
        ...
        const isActive = !hasStatusField || // No status field = assume active
        ...
        return {
            valid: true,
            userId: session.userId || session.user_id || session.user || session._id?.toString()
        };
    } catch (error) {
